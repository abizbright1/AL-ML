...
benchmarks, which use several hundred subjects. The MPS
...
multi-modal MRI reconstruction framework. On BraTS 2021, PP-MAE
achieves Dice scores of 0.8383 (TC) and 0.7903 (ET) — the
highest among five compared models — despite recording the lowest
PSNR (28.06 dB). This PSNR paradox demonstrates that pixel-level
reconstruction fidelity is an incomplete proxy for diagnostic
utility in tumour-focused imaging tasks. Our ablation further
shows that PathologyLoss alone deteriorates ET performance when
applied to architectures without tumour-oriented inductive bias,
underscoring the necessity of coherent multi-component design.
We hope these findings encourage the community to evaluate
reconstruction models on the downstream tasks they serve, rather
than on general-purpose image quality metrics alone.
